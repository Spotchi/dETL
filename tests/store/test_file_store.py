...
from detl.core.resultwrapper import FunctionResultWrapper, Returner
    ...
    def test_importing_a_result_group(self):
        ...
        with filestore.as_default():
            for rg in self.result_groups:
                # generate 2 results for each ResultGroup
                for param_val in range(2):
                    # TODO : separate parameters that are identifiable by themselves
                    result_wrap = FunctionResultWrapper(rg, Returner(param_val, str(param_val)))

                    # TODO :MOVE this in wrapper
                    run_uuid = uuid.uuid4().hex

                    run = result_wrap.run(run_uuid)

                    result_value = run.data
